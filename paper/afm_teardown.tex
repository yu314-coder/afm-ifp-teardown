...
\textbf{What still does not work.} A coherent from-weights forward pass. The remaining gap is narrow
and identified: the ANE emits residual-writing projections under a coefficient ordering
($\mathrm{OutTrans}{=}1$) that no available compiler reproduces for an isolated operator
(Finding~\ref{find:outtrans}). Five of seven weight roles per layer are decoded correctly; two are
not.

\textbf{On negative results.} Four instruments used during this work produced confident-looking
conclusions that later proved unfounded, each because the null hypothesis destroyed more structure
than the hypothesis under test. Those retractions are recorded in place rather than removed
