...
de-swizzle, and architecture---while the feed-forward stage exposes the true boundary of the
teardown: the per-layer expert wiring and the instruction-following expert-selection mask are
\emph{compiled into the Neural Engine} rather than shipped as data, so the recovered model is
\emph{component-complete but wiring-incomplete}. All findings derive from read-only
inspection; no code was executed on the models and no signing material was bypassed.
\end{abstract}
...
\subsection{The router, and the missing constant table}
Two artefacts expected of an MoE model are absent from the shipped IFP graph, and both are
resolved rather than missing. \emph{(i)~The router.} The exported adapter carries the token
\code{ifp1\_\allowbreak r48\_\allowbreak prompt\_\allowbreak opt\_\allowbreak dense\_\allowbreak only}: instruction-following pruning
was run once at export time and \emph{baked} the per-instruction expert mask into a fixed
dense expert set, so no runtime mask predictor ships. \emph{(ii)~The per-tensor offset table.}
The compile-time \code{ifp\_\allowbreak constant\_\allowbreak table\_\allowbreak ifp1\_\allowbreak r48.\allowbreak json} is not in the
assets, but its runtime equivalent is: the shipped \code{metadata.\allowbreak bin} \code{BBBB}
swizzler is exactly the constant$\to$offset scatter table (Appendix~\ref{app:bbbb}), and the
\code{MLIR22.\allowbreak 0.\allowbreak 0} MPSGraph bytecode tags each of the $396$ \code{ifp\_\allowbreak constant}
symbols with its full layer/role location hierarchy (e.g.\ \code{lut\_\allowbreak to\_\allowbreak dense}
$\to$ \code{MultiOutputLinear} $\to$ \code{TransformerFeedForward} $\to$
\code{TransformerLayer}), giving the per-tensor semantics directly.
...
\begin{finding}[the recovered weights are correct]\label{find:attnstable}
The attention-only forward pass is \emph{stable}: the residual-stream norm grows smoothly and
sub-exponentially ($17\!\to\!267$ across 44 layers), exactly the behavior of a correctly-wired
pre-norm transformer. Because a pre-norm block re-normalizes its own input, any decode or
de-swizzle error would manifest as divergence; the smooth growth therefore \emph{independently
validates} the 4-bit LUT codec, the ANE $8{\times}128$ de-swizzle (Eq.~\ref{eq:deswz}), the
attention tensor mapping, and the architecture---a stronger check than the structure ratio of
\S\ref{sec:recover}.
\end{finding}

\begin{finding}[the FFN wiring is a recoverable sequential layout]\label{find:wiring}
Adding a \emph{mis-aligned} feed-forward stage diverges exponentially---a pre-norm layer adds
a bounded increment regardless of $\lVert h\rVert$, so divergence can only come from reading
the \emph{wrong} tensor. The correct layout, however, requires no shipped offset table: the
396 \code{ifp\_\allowbreak constant} weights are stored \emph{sequentially in symbol-index
order}, so $\text{offset}(N)=\text{BACK\_END}+\sum_{i<N}\lvert c_i\rvert$. The per-constant
size follows from \code{num\_\allowbreak ffns}${=}3$, which splits each layer's $56064$-wide MoE
bank into three $18688$-wide modules; the endpoint constraint (the cumulative layout must fill
the expert region up to the embedding tail) pins the effective module width to
$\approx\!16384$. Under this layout the first $120$ constants ($\approx$13 layers) decode
correctly ($R{>}4$), and---decisively---\textbf{the full forward pass (attention $+$ MoE-SwiGLU
$+$ self-routing) is stable}: the residual norm grows smoothly $86\!\to\!408$ over ten layers,
the correct pre-norm signature. The wiring is therefore recovered, not ANE-locked; the
remaining $\sim$10\% of constants drift where a module size changes, a bounded refinement.
\end{finding}

\begin{finding}[the router architecture is recovered]\label{find:routerarch}
The unshipped selector is \emph{not} opaque microcode. The experts asset
(\code{project\_\allowbreak experts.\allowbreak mlasset/\allowbreak main-unspecialized.\allowbreak odix}) defines
\code{ExportableExpertSelector} as
$\text{mask}=\mathrm{top\text{-}k}\big(\mathrm{sigmoid}(\mathbf{h}\,W_{\text{proj}})\big)$
over a $[1536{\to}219]$ projection (\code{coreai.\allowbreak sigmoid}), selecting $10$ active $+\,4$
shared experts. Only the projection weight $W_{\text{proj}}$ remains to be located; a
self-routing top-$k$ surrogate already yields a stable forward pass.
\end{finding}

\begin{finding}[the router is dynamic, not stored]\label{find:router}
\code{config.\allowbreak json} sets \code{expert\_\allowbreak selection\_\allowbreak frequency}${=}32$: the
Instruction-Following-Pruning mask that selects $\approx$14 of 219 experts per sparse layer is
\emph{recomputed every 32 tokens} from the running context. It is therefore not a shipped
table but a live ANE computation; exhaustive search of the rasterized file, the
\code{MLIR22.\allowbreak 0.\allowbreak 0} attribute section ($840{,}956$ attributes), the
\code{0xBEEFFACE} ANE binary, and a full process core dump finds it in none of them. Summing
all experts diverges ($\approx$17$\times$ overcount); a self-routing top-$k$ surrogate is
stable but is not Apple's selector.
\end{finding}

Table~\ref{tab:runnable} summarizes the boundary. Everything Apple ships as
\emph{self-validating data}---weights, architecture, codec, norm values, tokenizer---is
recovered; the residual gap is exactly the \emph{assembly metadata} (per-layer FFN offsets,
attention$\leftrightarrow$FFN pairing, the dynamic expert mask), which exists only as ANE
behavior. The recovered model is thus \emph{component-complete but wiring-incomplete}: its
bricks are verified, its blueprint is on the coprocessor.
...
Attention path (stable) $+$ FFN wiring    & Router weight $W_{\text{proj}}$ \\
FFN sequential layout ($86\!\to\!408$ stable) & Embedding / output-head decode \\
Router \emph{architecture}; codec; tokenizer & Dynamic mask (or self-route surrogate) \\
\bottomrule
\end{tabular}
\caption{Revised recovery status. The forward pass runs stably on the recovered weights with
the sequential FFN wiring; the router \emph{architecture} is recovered. What remains are
bounded extraction tasks, not ANE-locked behavior---correcting the earlier verdict that the
wiring was irrecoverable.}\label{tab:runnable}
\end{table}

A practical corollary: the only faithful way to \emph{execute} this exact model is Apple's
\code{FoundationModels} runtime, which holds the ANE-baked wiring by construction. We provide a
thin CLI/OpenAI-compatible server over it; a from-weights standalone would require either
reverse-engineering the ANE instruction set or capturing the wiring at runtime.
