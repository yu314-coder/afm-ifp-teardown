...
\author{Yu Yao-Hsing}
\address{\href{mailto:euler.yu@gmail.com}{euler.yu@gmail.com} \quad\textbullet\quad
\href{https://github.com/yu314-coder/afm-ifp-teardown}{github.com/yu314-coder/afm-ifp-teardown}}
\date{Revision of \today. Asset build \texttt{13M204130}; catalogue re-checked on
macOS~27.0 build \texttt{26A5378n}.}
...
\begin{abstract}
We present a static teardown of Apple's on-device generative-model asset bundle,
\code{com.\allowbreak apple.\allowbreak MobileAsset.\allowbreak UAF.\allowbreak FM.\allowbreak GenerativeModels}, as shipped to a
macOS~26/27 machine, together with an attempt to reconstruct a running forward pass from the
recovered tensors. The catalogue holds $124$ signed assets across six model families; we document
the container stack (Cryptex images, the \code{BBBB} weight container, the \code{odix} executable,
the ANE \code{.hwx} binary, the MPSGraph package), the \code{odix} reference scheme, and two
language backbones---a dense ${\sim}3$\,B model and a sparse Mixture-of-Experts with
instruction-following pruning---plus ${\sim}90$ task adapters, guardrail models, draft models and
server variants.

The weight codec is $4$-bit LUT palettization with a per-block \textrm{fp16} scale under an ANE tile
permutation. We reverse it, close the parameter budget exactly against the physical file
($9.862$\,B parameters, $100.00\%$ of the shipped weight bytes), and validate the decoder by
round-trip against known weights at correlation $0.981$---the $4$-bit quantization floor. Two
obstacles previously believed fatal dissolve: the router$\rightarrow$expert map is mathematically
irrelevant to the output because the pruned feed-forward is an ungated permutation-invariant sum,
and the ``missing constant table'' has a shipped runtime equivalent. A later OS build additionally
ships the $3$B token embedding that earlier builds withheld.

A coherent from-weights forward pass nevertheless does not follow. We localize the obstruction to a
single ANE storage convention---the coefficient ordering used by residual-writing projections---which
no available compiler reproduces for an isolated operator. We also retract four of our own
intermediate results, each traceable to a null hypothesis that destroyed more structure than the
hypothesis under test; we consider that failure mode the most transferable finding in this work. The
analysis is read-only and no signing material was bypassed; no weights or tokenizer data are
reproduced here or published.
\end{abstract}
...
This report documents a static teardown of the on-device model assets that ship with Apple
Intelligence, together with an attempt to reconstruct a running forward pass from the recovered
tensors. It is written to be read in either direction: as a description of the shipped artefacts
(\S\ref{sec:containers}--\S\ref{sec:pipeline}), or as a record of which reconstruction claims
survived scrutiny and which did not (\S\ref{sec:reconstruct}).

\textbf{What the assets contain.} The \code{UAF.\allowbreak FM.\allowbreak GenerativeModels} catalogue holds
$124$ signed assets spanning six model families. The weight codec is $4$-bit LUT palettization with
a per-block \textrm{fp16} scale, stored under an Apple Neural Engine tile permutation. Both are
reversible: the parameter budget closes \emph{exactly} against the physical file
($9.862$\,B parameters, $100.00\%$ of the shipped weight bytes, zero non-finite tensors), and the
decoder is validated by round-trip against known weights at correlation $0.981$---the $4$-bit
quantization floor.

\textbf{What that means in practice.} The shipped assets are sufficient to recover the model
weights, the architecture, the tokenizer, the router, and---as of macOS~27.0 build
\code{26A5378n}---the $3$B token embedding, which earlier builds did not ship. Two obstacles that
were previously believed fatal turned out not to be: the instruction-following-pruning
router$\rightarrow$expert map is \emph{mathematically irrelevant} to the output
(Finding~\ref{find:ungated}), and the ``missing constant table'' has a shipped runtime equivalent
(Finding~\ref{find:metabin}).

\textbf{What still does not work.} A coherent from-weights forward pass. The obstacle is the
element$\rightarrow$position \emph{ordering} of the recovered tensors, and \S\ref{sec:latecycle}
reports it as unresolved for every weight role rather than for one. The file format itself is
settled: the tile payload begins at byte $+96$, not $+128$---a real decode defect found by its cause
and proven structurally, since all examined tiles carry exactly zero in their final $32$ bytes, and
confirmed on a second independent build whose weight map reproduces the first \emph{bit-exactly}
(Finding~\ref{find:p96}). The seven-role decomposition and every tensor shape are confirmed from
Apple's own shipped \code{manifest.plist}. All recovered tensor \emph{values} are trained matrices.
Only their arrangement is unknown (Observation~\ref{obs:revised}).

\textbf{On negative results.} This is the most transferable part of the report. Instruments used
during this work repeatedly produced confident conclusions that later failed---first weight
statistics whose null hypothesis destroyed more structure than the hypothesis under test
(Findings~\ref{find:picooracle}, \ref{find:vacuous}, \S\ref{sec:embretract}), then a composition
$z$-score (\S\ref{sec:retract-basis}), then a likelihood-based replacement. The last of these is
characterised rather than merely withdrawn: applying the best surviving candidate ordering to one
layer at a time improves $14$ of $24$ layers, and a \emph{random} permutation improves $14$ of $24$
(Finding~\ref{find:nopower}). Perturbing the axis at random helps half the time, which is the
signature of a tensor already contributing noise---so no ranking among permutations carries
information, and the earlier claim that six of seven roles were positively confirmed does not
survive. Ground-truth activations would settle the question directly, and \S\ref{sec:latecycle}
reports that route closed on measurement as well: the values never enter user-process memory, and the
runtime logs that would expose the layout are redacted behind a signing gate.

\subsection*{Findings at a glance}

\begin{longtable}{@{}p{0.055\textwidth}p{0.60\textwidth}p{0.28\textwidth}@{}}
\toprule
...
\midrule
\endhead
\ref{find:codec} & Weight codec is $4$-bit LUT palettization $\times$ per-block \textrm{fp16} scale & Established \\
\ref{find:budget} & Parameter budget closes exactly against the physical file & Established \\
\ref{find:recon} & Codec is necessary, then sufficient, for reconstruction & Established \\
\ref{find:full} & Budget closure bounds the search fan-out & Established \\
\ref{find:deswz} & The ANE spatial permutation, reversed & Established \\
\ref{find:hwx} & The \code{.hwx} program holds no weights & Established (negative) \\
\ref{find:ref} & \code{odix} reference scheme: self-relative signed offsets & Established \\
\ref{find:attnstable} & Recovered weights cross-validated against Apple's own toolchain & Established \\
\ref{find:normfold} & Per-layer norm $\gamma$ is compile-time folded & Established \\
\ref{find:ungated} & Expert-selection map is a \emph{false} blocker & Established \\
\ref{find:metabin} & ``Missing constant table'' $=$ the shipped \code{metadata.bin} & Established \\
\ref{find:deployedffn} & Deployed sparse-FFN shape recoverable from \code{metadata.bin} & Established \\
\ref{find:aneint} & Intermediate activations are ANE-internal & Established (negative) \\
\ref{find:dyncap} & Dynamic capture localizes the live embedding & Established \\
\ref{find:embrecover} & Host capture recovers the small model's embedding & Established \\
\ref{find:template} & Real input is lowercased, chat-templated, guard-model-preceded & Established \\
\ref{find:posread} & ANE coefficient z-order, positionally read, round-trip validated & Established \\
\ref{find:outtrans} & $\mathrm{OutTrans}{=}1$ marks the residual-writing projections & Established; remainder open \\
\ref{find:embed} & Embedding absent from the weight file & Superseded (\ref{find:embshipped}) \\
\ref{find:embformat} & $3$B ships no embedding file & Superseded (\ref{find:embshipped}) \\
\ref{find:3bane} & $3$B embedding is ANE-locked & Superseded (\ref{find:embshipped}) \\
\ref{find:embshipped} & A later OS build \emph{ships} the $3$B embedding & Established; layout open \\
\ref{find:ablate} & Full-forward ablation: KV geometry, ungated sum & Established; oracle caveated \\
\ref{find:picooracle} & Single-token rank oracles manufacture false positives & \textbf{Retraction} \\
\ref{find:vacuous} & A weight-statistics test decisive on Qwen is vacuous on AFM & \textbf{Retraction} \\
\ref{find:picoffn} & The pico forward's defect is one block & Amended (\ref{find:vacuous}) \\
\bottomrule
\end{longtable}
...
\toprule
\textbf{Problem} & \textbf{Why it is open} & \textbf{Kind} \\
\midrule
$\mathrm{OutTrans}{=}1$ coefficient order & Chosen by the ANE scheduler from full-graph context; no
available compiler emits it for an isolated operator & Tooling limit \\
$3$B embedding element layout & File present and exactly sized; row stride and $8$-lane structure
proven, per-token vectors not yet recovered & Decode problem \\
$56{,}064$-wide FFN alignment ($3$B) & No per-layer ground truth: activations never leave the ANE &
Information limit \\
Which-$46$-of-$219$ pruning list & Consumed at export time; not in any shipped asset. Irrelevant to
output correctness (\ref{find:ungated}) & Information limit \\
True layer count of the draft model & $38$ coefficient blocks beyond the base architecture form a
KV-reusing layer-shaped unit after the last mapped layer & Open \\
\bottomrule
\end{tabular}
\end{center}
...
All figures come from mounting the shipped, unencrypted Cryptex disk images read-only and parsing
their contents. No signing material was bypassed, no protection was circumvented, and no code was
executed on the models to obtain the static results. One step is dynamic: a user-privileged
\code{lldb} core dump of the reader's own running process on their own hardware, used to observe an
embedding buffer that the CPU produces in host memory (\S\ref{find:embrecover}).

No model weights, tokenizer data, or other Apple-proprietary artefacts are reproduced in this
document or in the accompanying public repository, which contains only original analysis and decoder
source. Quantities reported here are measurements, offsets, and statistics---not content.
...
Table~\ref{tab:families} groups the assets; six families are present. The catalogue held $118$ assets when first inventoried and $124$ on macOS~27.0 build \code{26A5378n}; \S\ref{find:embshipped} covers the delta.
...
Context                & 8192 \\
Vocabulary             & $262{,}144$ (\S\ref{sec:modelspec}) \\
...
\subsection{The recovered tensors are correct}\label{sec:tensorsok}
...
\subsection{Two blockers that dissolved}\label{sec:dissolved}
...
\subsection{The observability boundary}\label{sec:observability}
...
\subsection{The embedding: three verdicts, the last one current}\label{sec:embretract}
...
\subsection{Running the reconstruction, and what the oracle can support}\label{sec:oracle}
...
\subsection{Instruments that failed, and why}\label{sec:instruments}
...
\subsection{Ground truth from the ANE compiler}\label{sec:groundtruth}

\begin{finding}[the ANE coefficient z-order, read positionally and validated by round-trip]\label{find:posread}
Finding~\ref{find:vacuous} concluded that only ANE ground truth could settle the pico weight
arrangement. That read has now been performed, at both of pico's tile geometries.
Pico's own \code{binary\_\allowbreak 0.hwx} gives the target configuration directly: the down projection is
\emph{four} ANE tasks of $\mathrm{Cin}{=}3200\!\to\!\mathrm{Cout}{=}256$ with
$\mathrm{OCGSize}{=}4$, $\mathrm{ActiveNE}{=}4$, $4$-bit palettization and \textbf{sixteen
coefficient banks}. This reinterprets the weight container: the ``blocks'' of the weight map are
per-task coefficient allocations and the ``tiles'' are the ANE's coefficient \emph{banks}, which
partition \emph{output} channels---so a bank holds $16$ outputs $\times$ \emph{all} inputs
($16\times3200 = 51200$ nibbles, matching the observed payload exactly), and the $16$ header scales
are one per output channel. The same arithmetic fixes the other class: an \code{N} tile is
$128$\,B header $+$ $8192$\,B payload $=16384$ nibbles, i.e. $16$ banks $\times$ $16$ out $\times$
$1024$ in.
Compiling digit probes at each geometry---weights whose $4$-bit index at $(o,i)$ encodes a base-$16$
digit of $o$ or $i$---and decoding every physical nibble yields a \textbf{perfect bijection at both}:
$256\times3200 = 819{,}200$ and $256\times1024 = 262{,}144$ distinct $(o,i)$ pairs, each satisfying
the identical closed form
\[ o \;=\; 16\,\mathrm{bank} + (\mathrm{slot} \bmod 16), \qquad i \;=\; \lfloor \mathrm{slot}/16 \rfloor, \]
exact for all $16$ banks. Notably there is \emph{no} input pair swap here, unlike the 3B's
$ig\oplus1$: the interleave differs at this geometry, which is why transplanting the 3B formula
produced noise.
Two practical corrections were required and are worth recording. All-zero/all-\code{F} probes cannot
isolate weight bytes, because a constant tensor collapses the palette to a single entry and every
index becomes $0$; payload positions must be taken structurally instead. And each digit must be
decoded through \emph{its own} per-bank codebook: probes whose digit spans all $16$ values compile to
an identity LUT, but \code{i8} (which reaches only $12$, since $3200<16^3$) compiles to
$[0,1,2,2,3,4,5,6,6,7,8,9,10,10,11,12]$, where index $\neq$ value.
\emph{The decoder is then validated by round-trip.} Compiling a conv with \emph{known} random weights
and decoding its coefficient stream with the recovered order returns the true weights at correlation
$\mathbf{0.981}$---the $4$-bit palettization floor, i.e. exact up to quantization. The scale location
is likewise confirmed on shipped tiles: codebook at $\mathrm{fp16}[0{:}16]$ and a genuine per-output
scale at $\mathrm{fp16}[32{:}48]$ (the \code{s} class carrying exactly $8$ there, matching $8$
outputs per bank). So the shipped decode---$\mathrm{codebook}[\mathrm{nibble}]\cdot
\mathrm{scale}[o]$ under the closed form above---has every component independently verified.
\end{finding}

\begin{finding}[\code{OutTrans}$=1$ marks the residual-writing projections, shrinking the open problem to two roles]\label{find:outtrans}
The reconstruction nevertheless does not run, and the reason is now precisely located. Every conv
this toolchain can compile emits $\mathrm{OutTrans}{=}0$, whereas pico's down projection ships
$\mathrm{OutTrans}{=}1$. Sweeping conv shapes and reproducing pico's $\mathrm{Mul}\!\to\!\mathrm{conv}$
fragment never triggers it: the transpose is chosen by the ANE scheduler from full-graph context, not
from shape or a flag, so it cannot be recreated from an isolated op.
Grouping all $1003$ coefficient-bearing tasks in pico's \code{hwx} by shape and mode, and then reading
the \emph{task order} within a layer, identifies exactly which projections use it. Four
$1024\!\to\!256$ $\mathrm{OutTrans}{=}1$ convs sit immediately after the attention softmax---the
\textbf{output projection}, $1024\!\to\!1024$ in four chunks---and four $3200\!\to\!256$
$\mathrm{OutTrans}{=}1$ convs sit immediately after the SwiGLU \code{Mul}---the \textbf{down
projection}. The gate and up matrices are $\mathrm{OutTrans}{=}0$. The rule is architectural:
\begin{quote}
$\mathrm{OutTrans}{=}1$ is used by exactly those ops that \emph{write into the residual stream}---the
attention output projection and the FFN down projection (and their LoRA up-projections). Everything
that reads the normed hidden state---$Q$, $K$, $V$, gate, up---is $\mathrm{OutTrans}{=}0$.
\end{quote}
Since $\mathrm{OutTrans}{=}0$ is precisely the mode the round-trip validates, \textbf{$Q$, $K$, $V$,
gate and up are already correctly decoded}; only the two residual-writing roles carry an unknown
ordering, most plausibly an output-channel permutation. Decoding the five validated roles correctly
and varying only $O$/down does move the oracle---an intra-bank transpose doubles the depth-$1$
correlation ($+0.081$ versus $+0.047$, at identical scaling, so not a magnitude artifact), and one
combination is the first whose \emph{full} $24$-layer forward does not collapse (\code{\_Paris} rank
$3749$ against a depth-$0$ baseline of $2213$, where the uniform decode reaches $158{,}351$). But every
correlation remains inside the ${\pm}0.06$ noise band and none exceeds the depth-$0$ correlation of
$+0.038$, so this is a structural narrowing, not a solution.
One further observation bears on the oracle itself rather than the ordering. The same trace shows
pico's graph carries inline \textbf{rank-$32$ LoRA adapters}: $1024\!\to\!32\!\to\!1024$ around the
output projection, $1024\!\to\!32\!\to\!3200$ with an \code{Add} for gate and for up, and
$3200\!\to\!32\!\to\!1024$ for down. The base asset's $998$ weight blocks are fully accounted for by
its $168$ base tensors, so those adapter weights live in the separate \code{lora\_\allowbreak 32}/\code{lora\_\allowbreak 48}
assets. If the captured-logit oracle was recorded with an adapter resident, the reconstruction is
missing an additive term that \emph{no} weight ordering can compensate for---which would make the
target unattainable by construction rather than by mis-assembly. This is untested, and it should be
resolved before further ordering work.
Finally, the reconstruction is exportable: the validated embedding, weights, norms and architecture
write to a GGUF that \code{llama.cpp} loads and runs at ${\approx}15$ tokens/s. Its output is
incoherent, exactly as the open ordering predicts---so the export is a correct container around
partially-ordered weights, useful as a harness into which any ordering fix drops directly.
\end{finding}
