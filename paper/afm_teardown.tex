...
\begin{finding}[the missing piece is wiring, not weights]\label{find:wiring}
Adding the feed-forward stage makes the residual stream diverge exponentially. This is
\emph{not} a weight or router error: a correct pre-norm layer adds a bounded increment
regardless of $\lVert h\rVert$, so divergence can only arise from reading the \emph{wrong}
feed-forward tensor. The expert region interleaves $12$ dense FFNs (intermediate $3072$) and
$32$ sparse MoE banks (intermediate $56064{=}219{\times}256$) with \emph{no in-band boundary
markers}: every offset decodes as a valid weight (structure ratio $R{\approx}40$ everywhere),
so the dense/sparse pattern and the per-layer FFN offsets cannot be recovered from the weight
stream. This layout is precisely the datum in the build-time
\code{ifp\_\allowbreak constant\_\allowbreak table\_\allowbreak ifp1\_\allowbreak r48.\allowbreak json}, whose runtime
form is compiled into the ANE program.
\end{finding}
...
\bottomrule
\end{tabular}
\caption{The recovery boundary. The left column is fully recovered and independently
validated; the right column is the compiled ``wiring'' that the Neural Engine executes but
never emits as data.}\label{tab:runnable}
\end{table}
